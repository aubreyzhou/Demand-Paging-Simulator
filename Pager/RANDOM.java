...
public class RANDOM {
	...
	public static void RANDOM(String[] args) throws FileNotFoundException {
		...
		while(time!=processNum*n) {
			...
			if(hitFrame==-1) {
				...
				//there is no empty frame, choose a random page to evict
				else {
					int rand = in.nextInt();
					//System.out.println("rand "+rand);
					int newFrame = rand%frameNum;
					int[]prev = frametable.get(newFrame);
					frametable.set(newFrame, pair);
					fault[currentProcess]++;
					residency[prev[0]]+=time-prev[2];
					eviction[prev[0]]++;
					//System.out.println(currentProcess+" references word "+word+" (page "+page+") at time "+time+": Fault, evicting page "+prev[1]+" from "+prev[0]+ " from frame "+newFrame);
				}
			}

...
public class LIFO {
	...
	public static void LIFO(String[] args) throws FileNotFoundException {
		...
		while(time!=processNum*n) {
			...
			if(hitFrame==-1) {
				...
				//there is no empty frame, choose the lastly loaded page to evict
				else {
					int[]prev = frametable.get(last);
					frametable.set(last, pair);
					fault[currentProcess]++;
					residency[prev[0]]+=time-prev[2];
					eviction[prev[0]]++;
					//System.out.println(currentProcess+" references word "+word+" (page "+page+") at time "+time+": Fault, evicting page "+prev[1]+" from "+prev[0]+ " from frame "+last);
				}
			}
